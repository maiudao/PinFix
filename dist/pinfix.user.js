...
function createUI(options) {
  ...
  function renderPopover(state) {
    ...
    popover.style.left = `${toolbarBox.right + 12}px`;
    popover.style.top = `${toolbarBox.top}px`;
  ...
  function renderAnnotations(state) {
    ...
    if (!currentCandidate || !state.open || state.tool !== 'select' || state.captureHidden) {
      ...
      candidate.style.left = `${currentCandidate.pageLeft}px`;
      candidate.style.top = `${currentCandidate.pageTop}px`;
      candidate.style.width = `${Math.max(currentCandidate.width, 12)}px`;
      candidate.style.height = `${Math.max(currentCandidate.height, 12)}px`;
    }
  ...
  function renderAnnotationBox(annotation, state) {
    ...
    box.style.left = `${annotation.rect.pageLeft}px`;
    box.style.top = `${annotation.rect.pageTop}px`;
    box.style.width = `${Math.max(annotation.rect.width, 8)}px`;
    box.style.height = `${Math.max(annotation.rect.height, 8)}px`;
    ...
    label.style.left = `${annotation.rect.pageLeft + annotation.rect.width - labelSize * 0.38}px`;
    label.style.top = `${annotation.rect.pageTop - labelSize * 0.38}px`;
  ...
  function renderMask(mask) {
    ...
    element.innerHTML = `
      <div class="pinfix-mask-label">MASK</div>
      <button class="pinfix-mask-delete" type="button" data-action="delete-mask" data-id="${mask.id}">x</button>
    `;
  ...
  function getNotePosition(annotation) {
    ...
    const defaultTop = annotation.rect.pageTop + annotation.rect.height + 10;
    const viewportBottom = window.scrollY + window.innerHeight;
    const shouldFlip = defaultTop + estimatedHeight > viewportBottom && annotation.rect.pageTop > window.scrollY + estimatedHeight;

    return {
      left: clamp(annotation.rect.pageLeft, 12, window.scrollX + window.innerWidth - 372),
      top: shouldFlip ? annotation.rect.pageTop - estimatedHeight - 10 : defaultTop
    };
  }
  ...
  function renderAnnotationNote(annotation, state) {
    ...
    card.innerHTML = `
      <div class="pinfix-note-head">
        ...
        <button class="pinfix-note-delete" type="button" data-action="delete-annotation" data-id="${annotation.id}">×</button>
      </div>
  ...
  function renderGlobalNotes(state) {
    ...
    globalPanel.innerHTML = `
      <div class="pinfix-global-head">
        ...
        <button type="button" class="pinfix-note-delete" data-action="close-global">×</button>
      </div>
      <textarea
      ...
      return `
        ...
        <div class="pinfix-section">
          ...
          ${buildChipRow('colorPreset', Object.keys(PINFIX_COLOR_PRESETS), state.settings.colorPreset, (key) => {
            ...
            return `<span class="pinfix-color-dot" style="background:${item.color}"></span>`;
          })}
        </div>
        <div class="pinfix-section">
          <div class="pinfix-section-title">${escapeHtml(t('styleWidth'))}</div>
          ${buildChipRow('lineWidth', Object.keys(PINFIX_LINE_WIDTHS), state.settings.lineWidth, (key) => key)}
        </div>
        <div class="pinfix-section">
          <div class="pinfix-section-title">${escapeHtml(t('styleLabelSize'))}</div>
          ${buildChipRow('labelSize', Object.keys(PINFIX_LABEL_SIZES), state.settings.labelSize, (key) => key)}
        </div>
        <div class="pinfix-section">
